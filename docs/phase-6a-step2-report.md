...
| ID | Category | Note |
|---|---|---|
| FL-6A2-001 | **[boilerplate/template]** | Path-based `vi.mock` couples the test harness to the pre-extraction file layout: mocking `../../roleplay/grading.ts` stops intercepting once the SUT imports the module via its package specifier, forcing test-file edits during an otherwise shim-only extraction. A package-specifier (or DI-based) mocking convention in the template would let feature code move without touching the harness. **Accepted:** adopt in `bray-app-template` harness next time the template is touched (not in 6A). |
| FL-6A2-002 | **[accepted convention]** | `migrationsDir: null` is the standard for feature packages extracted from apps with pre-extraction migration history — historical migrations stay in the shell; the premium app generates its own `0000` from composed schema. Flashcards follows the same pattern (shell keeps `0000`–`0002`). |
